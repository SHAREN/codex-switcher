...
use crate::commands::{
    add_account_from_auth_json_text, cancel_login, check_codex_processes, complete_login,
    delete_account, export_accounts_full_encrypted_bytes, export_accounts_slim_text,
    get_active_account_info, get_cached_usage, get_codex_activity, get_masked_account_ids,
...
pub fn run_lan_server(host: &str, port: u16) -> anyhow::Result<()> {
    ...
    println!("Codex Switcher LAN server listening on http://{address}");
...
async fn invoke_web_command(command: &str, payload: Value) -> Result<Value, String> {
    match command {
        ...
        "sync_live_auth" => to_json(sync_live_auth().await?),
        "get_cached_usage" => to_json(get_cached_usage().await?),
        "get_usage" => {
        ...
        "get_codex_activity" => to_json(get_codex_activity().await?),
        "start_codex_app" => to_json(start_codex_app().await?),
        "stop_codex_app" => to_json(stop_codex_app().await?),
